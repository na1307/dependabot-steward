 ...
 */
function appFn(app: Probot): void {
    ...
    app.on('check_suite.completed', async context => {
        const payload = context.payload // The full webhook payload
        const owner = payload.repository.owner.login // Repository owner's login
        const repo = payload.repository.name // Repository name
        ...
        const processMerge = await (async () => {
            ...
            if (!pr) {
                console.log('No pull request found in the check suite.')
                return false
            }

            // Ensure the head and base repositories are the same (i.e., not a fork)
            if (pr.base.repo.id !== pr.head.repo.id) {
                console.log('Pull request is from a fork, skipping auto-merge.')
                return false
            }
            ...
            if (validSuites.some(cs => cs.status !== 'completed')) {
                console.log('Some check suites are not yet completed.')
                return false
            }
        ...
        if (processMerge) {
            ...
            try {
                ...
                // Ignore. There may be remaining rules.
            }
        }
    })
}
